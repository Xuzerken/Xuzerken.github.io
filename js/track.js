...
//     event_click                   点 Events 列表条目 → /events/<slug>/（props.slug/title）
//     talk_deck_open                打开一份 deck（JS 直发；props.deck）
//     talk_section_view             进入 deck 的某一节（JS 直发；props.deck/section/title）
...
//     talk_notes_toggle             演讲者备注浮层开合（JS 直发；props.state）
//     talk_deeplink_copy            复制当前页深链（JS 直发；props.hash）
//   stage 3 深入消费（单条成果）
...
//     event_link_click              活动详情页外链（官网/可查证来源；props.slug）
//     event_slides_click            活动详情页「幻灯片」入口（props.slug/talk_title）
//     talk_paper_link_click         从 deck 内点论文外链（props.deck/slug）
//     talk_pdf_download             deck PDF 下载（离线备份/带走；props.deck）
//   stage 4 资产/联系
//     cv_download                   CV PDF 下载
//     contact_click                 邮箱 / GitHub / Scholar / service 外链（props.target ∈ email|github|scholar|service）
//     talk_collaborate_click        结尾「合作/联系」号召（props.deck/target）
